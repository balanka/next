...
import type { IQuery, ITableProps, ITableState, Row } from './props/tableProps';
import{ makeStyles ,  Table as MTTable} from '@material-ui/core';
...
> {
  ...
  tableContainerDiv: React.RefObject<HTMLDivElement> | undefined = undefined;
    ...
    this.setState(
      {
        ...
        width: this.tableContainerDiv?.current?.scrollWidth ?? 0,
      },
      () => {
  ...
  setDataManagerFields(props: ITableProps<T>, isInit?: boolean) {
  ...
  getProps(props: ITableProps<T>) {
    ...
    const localization = {
      ...Table.defaultProps.localization.body,
  ...
  isRemoteData = (props?: ITableProps<T>) =>
    !Array.isArray((props || this.props).data);

  isOutsidePageNumbers = (props: ITableProps<T>) =>
  ...
  onChangeGroupOrder = (groupedColumn: Column<T>) => {
    this.dataManager.changeGroupOrder(groupedColumn.tableData.id);
    this.setState(this.dataManager.getRenderState());
  };

  onChangeOrder = (orderBy: 'acs' | 'desc', orderDirection: string) => {
  ...
  onQueryChange = (query: IQuery<T>, callback?: any) => {
    ...
    this.setState({ isLoading: true, errorState: undefined }, () => {
      this.props
        ...
        .catch(error => {
          const localization = {
            ...Table.defaultProps.localization,
  ...
  renderFooter() {
    ...
    if (props.options.paging) {
      const localization = {
        ...Table.defaultProps.localization.pagination,
    ...
    >
      {props.options.header && (
        <props.components.Header
          ...
          localization={{
            ...Table.defaultProps.localization.header,
        ...
        >
          ...
          {props.options.toolbar && (
            <props.components.Toolbar
              ...
              localization={{
                ...Table.defaultProps.localization.toolbar,
          ...
          {props.options.grouping && (
            <props.components.Groupbar
              icons={props.icons}
              localization={{
                ...Table.defaultProps.localization.grouping,
...
function ScrollBar({ double, children }: ScrollBarProps) {
